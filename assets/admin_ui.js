document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!settingsForm || settingsForm.dataset.adminUiInitialized === 'true') {
        return;
    }

    settingsForm.dataset.adminUiInitialized = 'true';

    function getFieldsetTitle(fieldset) {
        const legend = fieldset.querySelector(':scope > legend');
        return legend ? legend.textContent.trim() : 'Ustawienia';
    }

    const templateFieldsets = [];
    settingsForm.querySelectorAll(':scope > fieldset').forEach(function (fieldset) {
        if (fieldset.id === 'quote-template-settings' || getFieldsetTitle(fieldset) === 'Domyślny szablon wyceny e-mail') {
            templateFieldsets.push(fieldset);
        }
    });

    templateFieldsets.slice(1).forEach(function (fieldset) {
        fieldset.remove();
    });

    settingsForm.querySelectorAll(':scope > fieldset').forEach(function (fieldset, index) {
        const title = getFieldsetTitle(fieldset);
