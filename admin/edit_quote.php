...
try {
    ...
    if (!$quote) {
        ...
        // Jeśli treść e-maila jest nadal pusta (np. nowa wycena), użyj szablonu z ustawień.
        if (empty($email_body_to_show)) {
            $email_body_to_show = get_setting('global_quote_body', $pdo, false);
            $email_subject_to_show = get_setting('global_quote_subject', $pdo, false);
        }
        ...
        if (empty($email_subject_to_show)) {
